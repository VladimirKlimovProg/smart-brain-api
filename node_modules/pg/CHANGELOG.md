...
### 7.11.0
- Add support for [connection_timeout](https://github.com/brianc/node-***REMOVED***/pull/1847/files#diff-5391bde944956870128be1136e7bc176R63) and [keepalives_idle](https://github.com/brianc/node-***REMOVED***/pull/1847).

### 7.10.0
- Add support for [per-query types](https://github.com/brianc/node-***REMOVED***/pull/1825).

### 7.9.0

- Add support for [sasl/scram authentication](https://github.com/brianc/node-***REMOVED***/pull/1835).

### 7.8.0

- Add support for passing [secureOptions](https://github.com/brianc/node-***REMOVED***/pull/1804) SSL config.
- Upgrade [pg-types](https://github.com/brianc/node-***REMOVED***/pull/1806) to 2.0.

### 7.7.0

- Add support for configurable [query timeout](https://github.com/brianc/node-***REMOVED***/pull/1760) on a client level.

### 7.6.0

- Add support for ["bring your own promise"](https://github.com/brianc/node-***REMOVED***/pull/1518)

### 7.5.0

- Better [error message](https://github.com/brianc/node-***REMOVED***/commit/11a4793452d618c53e019416cc886ad38deb1aa7) when passing `null` or `undefined` to `client.query`.
- Better [error handling](https://github.com/brianc/node-***REMOVED***/pull/1503) on queued queries.

### 7.4.0

- Add support for [Uint8Array](https://github.com/brianc/node-***REMOVED***/pull/1448) values.

### 7.3.0

- Add support for [statement timeout](https://github.com/brianc/node-***REMOVED***/pull/1436).
...
- [You can now supply both a connection string and additional config options to clients.](https://github.com/brianc/node-***REMOVED***/pull/1363)
...
[Please see here for a migration guide](https://node-***REMOVED***.com/guides/upgrading)

#### Enhancements

- Overhauled documentation: [https://node-***REMOVED***.com](https://node-***REMOVED***.com).
...
- Add support for [parsing `replicationStart` messages](https://github.com/brianc/node-***REMOVED***/pull/1271/files).
...
- Replace internal pooling code with [pg-pool](https://github.com/brianc/node-pg-pool). This is the first step in eventually deprecating and removing the singleton `pg.connect`. The pg-pool constructor is exported from node-***REMOVED*** at `require('pg').Pool`. It provides a backwards compatible interface with `pg.connect` as well as a promise based interface & additional niceties.
...
- Make the query object returned from `client.query` implement the promise interface. This is the first step towards promisifying more of the node-***REMOVED*** api.
...
- Add option to parse JS date objects in query parameters as [UTC](https://github.com/brianc/node-***REMOVED***/pull/943)

### v4.4.0

- Warn to `stderr` if a named query exceeds 63 characters which is the max length supported by ***REMOVED***.
...
- Support for additional error fields in ***REMOVED*** >= 9.3 if available.

### v4.1.0

- Allow type parser overrides on a [per-client basis](https://github.com/brianc/node-***REMOVED***/pull/679)
...
- Include support for (parsing JSONB)[https://github.com/brianc/node-pg-types/pull/13] (supported in ***REMOVED*** 9.4)
...
- Include port as connection parameter to [unix sockets](https://github.com/brianc/node-***REMOVED***/pull/604)
...
- Allow [pool](https://github.com/brianc/node-***REMOVED***/pull/591) to be configured

### v3.1.0

- Add [count of the number of times a client has been checked out from the pool](https://github.com/brianc/node-***REMOVED***/pull/556)
- Emit `end` from `pg` object [when a pool is drained](https://github.com/brianc/node-***REMOVED***/pull/571)

### v3.0.0

#### Breaking changes

- [Parse the DATE PostgreSQL type as local time](https://github.com/brianc/node-***REMOVED***/pull/514)

After [some discussion](https://github.com/brianc/node-***REMOVED***/issues/510) it was decided node-***REMOVED*** was non-compliant in how it was handling DATE results. They were being converted to UTC, but the PostgreSQL documentation specifies they should be returned in the client timezone. This is a breaking change, and if you use the `date` type you might want to examine your code and make sure nothing is impacted.

- [Fix possible numeric precision loss on numeric & int8 arrays](https://github.com/brianc/node-***REMOVED***/pull/501)
...
- [Fix to input array of dates being improperly converted to utc](https://github.com/benesch/node-***REMOVED***/commit/c41eedc3e01e5527a3d5c242fa1896f02ef0b261#diff-7172adb1fec2457a2700ed29008a8e0aR108)
...
- [Query no longer emits `end` event if it ends due to an error](https://github.com/brianc/node-***REMOVED***/commit/357b64d70431ec5ca721eb45a63b082c18e6ffa3)
...
- [Supercharge `prepareValue`](https://github.com/brianc/node-***REMOVED***/pull/555)
...
- [Domain aware connection pool](https://github.com/brianc/node-***REMOVED***/pull/531)

If domains are active node-***REMOVED*** will honor them and do everything it can to ensure all callbacks are properly fired in the active domain. If you have tried to use domains with node-***REMOVED*** (or many other modules which pool long lived event emitters) you may have run into an issue where the active domain changes before and after a callback. This has been a longstanding footgun within node-***REMOVED*** and I am happy to get it fixed.

- [Disconnected clients now removed from pool](https://github.com/brianc/node-***REMOVED***/pull/543)

Avoids a scenario where your pool could fill up with disconnected & unusable clients.

- [Break type parsing code into separate module](https://github.com/brianc/node-***REMOVED***/pull/541)
...
- Add support for [application_name](https://github.com/brianc/node-***REMOVED***/pull/497)

### v2.10.0

- Add support for [the password file](http://www.***REMOVED***ql.org/docs/9.3/static/libpq-pgpass.html)

### v2.9.0

- Add better support for [unix domain socket](https://github.com/brianc/node-***REMOVED***/pull/487) connections
...
- [Add support for excapeLiteral and escapeIdentifier in both JavaScript and the native bindings](https://github.com/brianc/node-***REMOVED***/pull/396)

### v2.1.0

- Add support for SSL connections in JavaScript driver
- this means you can connect to heroku ***REMOVED*** from your local machine without the native bindings!
- [Add field metadata to result object](https://github.com/brianc/node-***REMOVED***/blob/master/test/integration/client/row-description-on-results-tests.js)
- [Add ability for rows to be returned as arrays instead of objects](https://github.com/brianc/node-***REMOVED***/blob/master/test/integration/client/results-as-array-tests.js)
...
For more information see https://github.com/brianc/node-***REMOVED***/pull/353
