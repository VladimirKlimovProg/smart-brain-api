...
- fix wrapIdentifier not being called in ***REMOVED*** alter column #2612
- fixes wrapIdentifier to work with ***REMOVED*** `returning` statement 2630 #2642
...
- Support ***REMOVED***ql connection uri protocol #2609
...
- Fix support for multiple schema names in in ***REMOVED*** `searchPath` #2340
...
- Fix ***REMOVED*** searchPath to be case-sensitive #2172
...
- Unit test for renaming column in ***REMOVED***ql #2099
...
- Added [`with`](#Builder-with) to builder for [common table expressions](https://www.***REMOVED***ql.org/docs/9.4/static/queries-with.html), #1599
...
- Alias ***REMOVED***ql -> ***REMOVED*** in connection config options
...
- Create/drop schema for ***REMOVED***, #511
...
- Bug fix for properly binding ***REMOVED***ql streaming queries, (#363)
...
- Streams are supported for selects, passing through to the streaming capabilities of node-mysql and node-***REMOVED***
...
- Fix regression in array bindings for ***REMOVED***ql (#228)
...
- Remove error'ed connections from the connection pool (#206), added support for node-***REMOVED***-pure (pg.js) (#200)
...
- Changed syntax for insert in ***REMOVED***ql, where the `id` is not assumed on inserts (#18). The second parameter of [insert](#Builder-insert) is now required to return an array of insert id's for the last insert.
